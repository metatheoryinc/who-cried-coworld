...
One other bot per private Wolf/Noble channel can coordinate alongside each public turn. Human Town and team chat remain open throughout discussion. Night coordination retains two rounds for all team bots. Moderator selection has a two-second limit within the 13-second turn; the player receives the remaining time. A failed, invalid or late moderator choice uses the deterministic fallback, which prioritizes human mentions then rotates by turn count. Every decision's first call gets the whole remaining window, less a 500 ms delivery margin (it was capped at 15 seconds until September 25, 2026, which timed out Kimi, GPT-OSS and DeepSeek on hosted runs). An early failure can retry once if at least two seconds remain. Timeouts and permanent HTTP request errors are not retried. Empty night-action requests return a legal empty action without calling an LLM. Missing votes/actions safely pass; missing speeches stay silent. No response closes a phase early.
...
Before play starts, the lobby leads with **Join the village** (in the table and in place of the chat composer) and hides the role and journal panels. After joining, each seat shows **You**, **Joined** (human), **AI ready**, or **Open seat**, and the clock counts down to auto-start once the first human has joined; this lobby status is sent only while the game is waiting. On desktop widths the board and chat stay inside the painted frame (about the top 78% of the window); between 761 and 1100 px wide the seats use two rows so the action panel stays visible. Up to 760 px wide (phones, including the ~277 px Softmax column) the page becomes a conversation-first shell: a top bar (phase, clock, **?** for How to play, and a role chip that opens your role, teammates, and journal), a strip of all nine seats numbered 1–9 (names from 400 px), and chat filling the rest. Decisions open in a bottom sheet with the stamp tray and a 3×3 seat grid; it collapses to a pill showing the clock. Tapping a seat otherwise opens its player card (status, known role, vote history). When a decision has a single stamp (the vote, or one night ability), tapping a player places it directly on every screen size. Chat names carry the same seat number and color, and the host's current speaker pulses. Game-over actions stay pinned at the bottom. The page uses `viewport-fit=cover` and adds the device safe-area insets to the page, interstitial, end-screen, and setup-guide spacing, so full-screen phones keep content clear of notches and the home indicator.
...
- The server keeps a human's latest legal vote or night choice as a draft until the timer ends; votes and night abilities are stamps: pick one up in the action panel, then click a glowing player card. Clicking a card with your stamp picks it up again (click another card to move it, the same card to remove it, or × in the tray); Esc cancels. Changes save automatically (Saving… / Saved) and count when the timer ends; no stamp means pass. Stamps use the original client's art (hoof vote, Wolf claw slashes, ability icons; a drawn knife) and land at a stable random spot and tilt on the card, with no ring. The stamp being applied becomes the mouse cursor over eligible players. **Skip vote** (the dove in the tray) passes explicitly; no mark is placed on any card. When the vote closes, every ballot flies from its voter to its target (passes are not stamped); the dusk screen waits for the stamps to land (about 3 seconds with nine voters), and the vote stays stamped through the night. Wolves get a Kill target stamp and a Knife stamp (starting on themselves), and see packmates' stamps faded with the packmate's seat number. Town chat shows game events (day start, vote result with revealed role, nightfall, dawn deaths); team channels show only night and day starts. On desktop your role is a compact card above the chat, **How to play** is the **?** next to the connection light, and the journal fills the rest of the board column, growing with the window. Journal entries are newest first: each vote leads with the outcome, then a tally and colored seat-chip ballots (your own highlighted); nights list deaths with revealed roles; your private results (Seer, Track Reader, Priest, Dairy Maid) are boxed with their ability icon. Chat channels are folder tabs with an icon and color (Town gold, Wolves red, Nobles blue); the panel edge and heading follow the open channel, and private channels say who can read them. Unopened tabs show an unread count (9+ max), highlighted with @ when a message names you; opening a channel marks a New divider above the first unread message. Read positions persist per browser tab. Human Wolves' snapshots include packmates' current night drafts. Reconnect/reload restores the seat, history, accepted choice and original deadline. A disconnect does not pause the game. Only one browser controls a seat at a time.
...
Mistral: 1,800 tokens, no reasoning option (Mistral Medium 3.1 does not support one); Qwen: 1,200 with mandatory reasoning enabled at minimal effort (excluded from the returned content); Gemini: 1,600 with minimal reasoning; DeepSeek/Kimi/GLM/MiMo: 1,600 with low reasoning; Llama: 800 without a reasoning option; other models: 1,000 with minimal reasoning. Every model except GPT-5.x (which does not support it) gets temperature 0.7. These are initial settings, not measured latency guarantees. Dialogue limits remain short. A `length` finish reason is logged as `output_limit`, distinct from invalid actions and network failures. Settings follow [OpenRouter reasoning controls](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens).
