...
export function chooseSpeaker(input:{roster:{slot:number;name:string;alive:boolean}[];counts:Record<number,number>;recent:number[];allNames?:string[];humanMessage?:{id:string;text:string}}){
 ...
 const mentioned=pool.filter(p=>{
  return !!input.humanMessage&&mentionsName(input.humanMessage.text,p.name,input.allNames??input.roster.map(r=>r.name));
 });
