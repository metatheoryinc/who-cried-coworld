import {z} from 'zod';
export type ModeratorInput={day:number;humanSlot:number;roster:{slot:number;name:string;alive:boolean}[];counts:Record<number,number>;eligibleSlots?:number[];recent:number[];humanMessage:{id:string;text:string}|null;transcript:{id:string;slot:number;text:string}[]};
...
export function validateModeratorChoice(raw:unknown,input:ModeratorInput){
 ...
 if(choice.slot===input.humanSlot||!input.roster.some(p=>p.slot===choice.slot&&p.alive))throw Error('Host chose an ineligible speaker');
 if(input.recent.length>=2&&input.recent.slice(-2).every(s=>s===choice.slot)&&input.roster.filter(p=>p.alive&&p.slot!==input.humanSlot).length>1)throw Error('Host repeated the same speaker three times');
 return choice;
}
