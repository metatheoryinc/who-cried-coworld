...
export class HumanSession extends Session {
 ...
 protected override enterDay(now:number){
  ...
  this.answeredHumanCursor=0;this.recentSpeakers=[];this.hostTurns={};this.restUntil={};
 ...
 protected override open(now:number,choice?:ModeratorChoice,moderated=false){
  ...
  const humanMessage=visible.filter(e=>e.day===this.state.day&&e.cursor>this.answeredHumanCursor&&e.payload.kind==='speech'&&e.payload.speech.slot===this.config.humanSlot).at(-1);
  const livingBots=this.living().filter(s=>s!==this.config.humanSlot);
  const ready= livingBots.filter(s=>(this.restUntil[s]??0)<=this.window);
  const eligibleSlots=ready.length?ready:livingBots;
  if(this.period==='discussion'&&this.moderator&&!moderated){
  ...
  for(const faction of ['wolf','noble']){const group=this.state.seats.filter(p=>p.alive&&p.slot!==this.config.humanSlot&&p.slot!==host?.slot&&(faction==='wolf'?p.faction==='wolf':p.role==='noble'));if(group.length)teamActors.add(group[this.window%group.length]!.slot);}
  for(const slot of this.living()){
   ...
   if(this.period==='discussion'||this.period==='coordination'){
    if(slot===this.config.humanSlot)continue;
   ...
   this.pending.set(slot,openRequest({slot,episodeId:this.episodeId,requestId:`r_${slot}_${index}`,observationId:`o_${slot}_${index}`,deadline:slot===this.config.humanSlot?this.deadline:Math.min(this.deadline,now+15000),request,visibleSpeechIds:publicSpeech}));
  }
 }
  ...
  for(const [slot,p] of [...this.pending].sort(([a],[b])=>a-b)){
   ...
   if(b.kind==='bid'){
    ...
    if(selected){
     ...
     if(replyTo){const answered=project(this.journal,'public').find(e=>e.id===replyTo);if(answered?.payload.kind==='speech'&&answered.payload.speech.slot===this.config.humanSlot)this.answeredHumanCursor=Math.max(this.answeredHumanCursor,answered.cursor);}
    }
 ...
 override disconnect(slot:number){if(slot!==this.config.humanSlot)super.disconnect(slot);}
 chat(slot:number,text:unknown,now:number):{status:'accepted'|'duplicate'|'rejected';message?:string}{
  ...
  if(slot!==this.config.humanSlot||!seat?.alive||this.state.result||this.phase==='waiting'||m.episodeId!==this.episodeId||m.phaseKey!==`${this.state.day}:${this.period}`)return reject('This chat window has closed.');
  ...
  const signature=JSON.stringify(m),prior=this.chatIds.get(m.id);
  ...
  this.chatIds.set(m.id,signature);this.chatCounts.set(key,count+1);this.lastChat.set(slot,now);
