...
import { NOTE_TARGET } from '../shared/actions.js';
/** This player's own ceiling for notes, well under the game's 512, so a wordy model is shortened rather than rejected. */
export const NOTE_LIMIT=300;
/** Cut a note to NOTE_LIMIT characters, at a word boundary when one is near, marking the cut with an ellipsis. */
export function shortenNote(note:string){
 const chars=[...note];if(chars.length<=NOTE_LIMIT)return note;
 const head=chars.slice(0,NOTE_LIMIT-1).join(''),space=head.lastIndexOf(' ');
 return (space>=NOTE_LIMIT-60?head.slice(0,space):head).trimEnd()+'…';
}
...
export function parseModelAction(content:string,o:Observation){
 ...
 // Notes are private confessionals: shorten a wordy one rather than lose the action. Speech is never shortened.
 if(shape)for(const key of ['summary','reason']){const v=(value as Record<string,unknown>)[key];if(typeof v==='string')(value as Record<string,unknown>)[key]=shortenNote(v);}
 const parsed=ActionBody.safeParse(value);
 if(!parsed.success){
  const long=Object.entries({text:480}).flatMap(([key,limit])=>{const v=(value as Record<string,unknown>)?.[key];return typeof v==='string'&&[...v].length>limit?[`${key} is ${[...v].length} characters; the limit is ${limit}`]:[];});
