...
export function playerSystemPrompt(o:Observation,personality=contestants[o.self.slot]?.personalityPrompt??''){
 ...
 const prefix=[
  ...
  o.request.kind==='vote'&&o.request.suspicion?'This vote also asks for your private suspicion: for every other living player, the probability from 0 to 1 that they are a wolf. It is private, never shown to anyone during the game, and never changes your vote. You are scored on honest, calibrated probabilities against the true roles, so exaggerating or hedging lowers your score. Use everything you know, including your private results and revealed roles. NewD3 starts with two wolves, so your probabilities should add up to roughly the number of wolves still alive.':'',
