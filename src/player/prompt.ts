...
export function playerSystemPrompt(o:Observation,personality=contestants[o.self.slot]?.personalityPrompt??''){
 ...
 const prefix=[
  ...
  // Hosted Wolves skipped night 1 to "let town settle"; a skipped kill gives Town a free day at the same numbers.
  o.self.faction==='wolf'&&['wolf_chat','night'].includes(o.request.kind)?'Skipping the night kill almost always helps Town: every night without a kill leaves Town a free day at the same numbers, and you win only by reaching parity. Kill every night unless you have a concrete reason not to, and agree with your team on both the target and the killer.':'',
  ...
  o.request.kind==='vote'&&o.request.suspicion?'This vote also asks for your private suspicion: for every other living player, the probability from 0 to 1 that they are a wolf. It is private, never shown to anyone during the game, and never changes your vote. You are scored on honest, calibrated probabilities against the true roles, so exaggerating or hedging lowers your score. Use everything you know, including your private results and revealed roles. NewD3 starts with two wolves, so your probabilities should add up to roughly the number of wolves still alive. Include exactly these players, once each: '+o.roster.filter(p=>p.alive&&p.slot!==o.self.slot).map(p=>`${p.slot} = ${p.name}`).join(', ')+'. Do not include yourself or eliminated players.':'',
