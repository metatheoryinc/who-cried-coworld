...
export const GameConfig=z.union([
 ...
 ConfigFields.extend({mode:z.literal('bots'),player_connect_timeout_seconds:z.number().int().min(1).max(180).default(30)}),
 ConfigFields.extend({mode:z.literal('human'),player_connect_timeout_seconds:z.number().int().min(1).max(300).default(300)}),
]).refine(c=>!c.setup||JSON.stringify(c.roles)===JSON.stringify(defaultRoles),'Choose a setup or a custom role deck, not both').refine(c=>episodeBudgetSeconds(c)<=3600,'Episode exceeds time budget');
