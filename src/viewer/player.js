...
function drawPlayers(){
 ...
 $('players').innerHTML=roster.map(p=>{
  ...
  return `<button class="player ${p.alive?'':'dead'} ${eligible.includes(p.slot)?'eligible':''}" data-slot="${p.slot}" ${eligible.includes(p.slot)?'':'disabled'} aria-label="${esc(p.name)}, ${esc(tag)}${votes.count?`, ${votes.own?'your locked vote':`${votes.count} votes, majority ${votes.majority}`}`:''}${eligible.includes(p.slot)?', select for vote':''}"><img src="${asset('base_playercard_shadow')}" alt=""><img class="${known?'known-role':'sheep'}" src="${asset(known?art[known]:'Player_sheep_base')}" alt="">${stamps}${eligible.includes(p.slot)?`<img class="vote-preview" src="${asset('vote_banner_town_hoof')}" alt="">`:""}${!p.alive?deathMark(cause):''}${badge}<span class="name">${esc(p.name)}</span><small>${esc(tag)}</small></button>`;
 }).join('');
