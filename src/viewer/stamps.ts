...
/** `skip` marks an explicit Skip vote (the dove); it sends the same pass as no vote. */
export type Placements=Partial<Record<StampId,number|null>>&{skip?:boolean};
...
export function placementsFrom(r:StampRequest,accepted:ActionBody|null):Placements{
 if(r.kind==='vote')return accepted?.kind==='vote'?accepted.target===null?{vote:null,skip:true}:{vote:accepted.target}:{};
...
export function place(p:Placements,r:StampRequest,id:StampId,slot:number,self:number):Placements{
 ...
 const next:Placements={...p,[id]:p[id]===slot?null:slot,...('skip' in p&&id==='vote'?{skip:false}:{})};
...
export function skipVote(p:Placements):Placements{return {...p,vote:null,skip:true};}
...
export function stampsOn(p:Placements,slot:number):StampId[]{return (Object.keys(p) as StampId[]).filter(id=>id!=='skip' as string&&p[id]===slot);}
