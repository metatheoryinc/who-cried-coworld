...
it('names the actual length when speech is over its limit',()=>{
 const chat={...o,request:{kind:'wolf_chat',turn:0,maxCharacters:480}} as typeof o;
 expect(()=>parseModelAction(JSON.stringify({kind:'wolf_chat',text:'x'.repeat(490),summary:''}),chat)).toThrow('text is 490 characters; the limit is 480');
});
it('shortens wordy notes at a word boundary instead of rejecting the action',()=>{
 const night={...o,request:{kind:'night',choices:[]}} as typeof o;
 const long=('word '.repeat(100)).trim();
 const summary=(parseModelAction(JSON.stringify({kind:'night',actions:[],summary:long}),night).body as {summary:string}).summary;
 expect([...summary].length).toBeLessThanOrEqual(300);expect(summary.endsWith('word…')).toBe(true);
 expect((parseModelAction(JSON.stringify({kind:'night',actions:[],summary:'x'.repeat(300)}),night).body as {summary:string}).summary).toBe('x'.repeat(300));
 expect([...(parseModelAction(JSON.stringify({kind:'night',actions:[],summary:'x'.repeat(400)}),night).body as {summary:string}).summary].length).toBe(300);
});
