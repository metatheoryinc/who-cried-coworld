...
it('uses the scripted baseline without credentials and never calls the provider',async()=>{
 const fetcher=vi.fn();const o=observation(),a=await llmAction(o,{model:'test',fetcher,allowScripted:true});
...
it('uses Bedrock without an OpenRouter key instead of silently playing scripted',async()=>{
 const o=observation();const bedrockSender=vi.fn().mockResolvedValue({body:new TextEncoder().encode(JSON.stringify({content:[{type:'text',text:JSON.stringify({kind:o.request.kind,text:'Bedrock reply.',summary:''})}],stop_reason:'end_turn'}))});
...
it('sends the same explicit JSON-only system prompt through both providers and accepts a fenced Bedrock reply',async()=>{
 ...
 const bedrockSender=vi.fn().mockResolvedValue({body:new TextEncoder().encode(JSON.stringify({content:[{type:'text',text:'```json\n'+JSON.stringify(body)+'\n```'}],stop_reason:'end_turn'}))});
 ...
 expect(JSON.parse(bedrockSender.mock.calls[0]![0].input.body).system).toBe(orPrompt);
...
it('rejects missing credentials instead of silently playing scripted',async()=>{
