...
it('logs rejected model text and gives the retry field-specific guidance',async()=>{
 const logs:any[]=[];
 const bad=JSON.stringify({kind:'wolf_chat',text:'x'.repeat(481),summary:''});
 ...
 expect(complete.mock.calls[1]![1]).toContain('text');
 expect(complete.mock.calls[1]![1]).toContain('480');
});
