...
it('folds the public replay identically to live delivery revealing only dead players’ roles',()=>{
 ...
 expect(foldEvents(live).roles).toEqual(Object.fromEntries(s.state.seats.filter(p=>!p.alive).map(p=>[p.slot,p.role])));
