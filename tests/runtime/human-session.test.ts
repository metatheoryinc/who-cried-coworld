...
const config=()=>GameConfig.parse({tokens:Array.from({length:9},(_,i)=>`t${i}`),players:Array.from({length:9},(_,i)=>({name:`Seat ${i}`})),mode:'human',setup:'A2',seed:'000102030405060708090a0b0c0d0e0f'});
...
it('expires old actions, preserves disconnected human windows and exports complete replay',()=>{
 const s=human(config());s.start(0);s.advance(150000);
...
it('runs scripted decisions with a fractional monotonic clock through transitions and game over',()=>{
 const s=human(config());let now=173.1234567;s.start(now);
...
it('hides fixed NewD3 selection and provides only authorized confirmed teammates',()=>{
 for(const setup of ['A2','A3'] as const){
  const s=human({...config(),setup});s.start(0);
...
it('lists all candidate setups without exposing the random draw or assignments',()=>{
 const s=human({...config(),setup:'random'});s.start(0);
...
it('exposes identical setup guides for every fixed and random NewD3 choice',()=>{
 const guides=['A1','A2','A3','B1','B2','B3','C1','C2','C3','random'].map(setup=>human(GameConfig.parse({...config(),setup})).snapshot(1,0).gameSetup);
