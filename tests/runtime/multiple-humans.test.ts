...
const config=(humanSlots:number[])=>GameConfig.parse({mode:'human',humanSlots,tokens:Array.from({length:9},(_,i)=>`t${i}`),players:Array.from({length:9},(_,i)=>({name:`Seat ${i}`})),setup:'A2',seed:'000102030405060708090a0b0c0d0e0f'});
...
it('gives multiple humans independent chat IDs, private team channels, and vote deadlines',()=>{
 const s=new HumanSession(config([0,1,5]),'episode');s.start(0);
...
it('runs an all-human game without asking a model to select a human speaker',()=>{
 const s=new HumanSession(config(Array.from({length:9},(_,i)=>i)),'episode');
...
it('prioritizes unanswered public messages from each human without losing another human’s turn',()=>{
 const s=new HumanSession(config([0,1]),'episode');s.start(0);
...
it('accepts a late human during voting with the remaining phase time',()=>{
 const s=new HumanSession(config([0]),'episode');s.start(0);s.advance(150000);
...
it('discards an in-flight moderator choice for a seat that has joined as human',async()=>{
 const s=new HumanSession(config([0]),'episode');let finish!:(choice:{slot:number;prompt:string})=>void;
